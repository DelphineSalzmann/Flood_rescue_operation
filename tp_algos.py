...
def tb3_control_fn(robotNo, tb3_poses, cf_poses, rmtt_poses, rms1_poses, obstacle_pose, ros_time):
    ...
    if not Entry_circle[i] and distance_firefighter_to_victim <= 0.4:
        Entry_circle[i] = True

    if Entry_circle[i] and distance_firefighter_to_victim > 0.4:
        Exit_circle[i] = True
    ...
    if distance_firefighter_to_objective < 0.3 and index_trajectory < len(trajectory) - 1:
        index_trajectory += 1

    # -----------------------
    vx, vy = anti_Collision_function(robotNo, tb3_poses, vx, vy, 0.3, 0.4)
...
def anti_Collision_function(robotNo, poses, vx, vy, Min_security_zone_robot, Avoid_collusion_gain):
    ...
    for j in range(len(poses)):
        if robotNo - 1 != j:
            distance_robotNo_Robotj = np.sqrt(
                (poses[0, j] - poses[0, robotNo - 1]) ** 2 + (poses[1, j] - poses[1, robotNo - 1]) ** 2)
            if distance_robotNo_Robotj < Min_security_zone_robot:
                vx_avoid = vx_avoid - (poses[0, j] - poses[0, robotNo - 1]) / distance_robotNo_Robotj ** 2 * (
                            1.2 - distance_robotNo_Robotj / Min_security_zone_robot)
                vy_avoid = vy_avoid - (poses[1, j] - poses[1, robotNo - 1]) / distance_robotNo_Robotj ** 2 * (
                            1.2 - distance_robotNo_Robotj / Min_security_zone_robot)
    ...
    if is_robot_in_range:
        ...
        ui_norm_min = 0.05
        wanted_norm = max(input_norm,
                          ui_norm_min)  # IF we wanted the robot to not move, it still have to avoid collision
